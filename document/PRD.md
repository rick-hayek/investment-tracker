...
- **交互与排序规则**：
  - **智能排序机制 (Intelligent Holdings Sorting)**：
    - **收藏置顶最高优先级**：用户在代币详情页点击右上角收藏星星（⭐）后，该代币立即优先排在资产列表最上方；此排序规则高于 0 持仓规则，即被收藏的代币即使持仓为 0 也排在所有未收藏代币的最前面；
    - **有效持仓优先**：在收藏与非收藏分组内，持有量大于 0 的资产排在前面，并默认按持仓市值（`marketValue`）从大到小降序排列，让核心主力仓位一目了然；
    - **清仓代币自动沉底**：未收藏且持仓量为 0（已全部卖出/已清仓）的代币自动放置在列表最后，避免干扰核心持仓视野，同时保留点击复盘历史流水的入口。
...
- **功能清单**：
  ...
  5. **交易所/平台管理 (Exchange & Platform Management - FR-SET-05)**：
     ...
     - **默认启用配置**：系统初始默认仅启用 `Binance` 与 `OKX`，`Coinbase` 与 `CoinGecko` 默认处于关闭状态（用户可随时在二级页面自由开启）；
