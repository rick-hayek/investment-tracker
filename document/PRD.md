...
#### 4. 交易记录编辑、更新与删除 (Edit, Update & Delete Transactions)
- **功能编号**：`FR-TX-EDIT-01`
- **详细规格**：
  1. **点击交易流水编辑**：用户点击「交易与调仓记录」中的任意一条流水卡片，弹出全屏/半屏编辑抽屉。
  2. **表单数据回填与修改**：
     - 自动带出该笔交易的全部原始信息：交易类型（买入/卖出）、所属平台、代币符号、成交单价、成交数量、发生时间戳（格式化为 `YYYY-MM-DD HH:mm`）及备注内容；
     - 允许用户对数量、价格、买卖方向、时间日期、备注进行完整编辑并实时联动推导总金额。
  3. **保存更新与取消更新**：
     - **取消与保存同一行排列**：编辑模式下，底部操作栏将「取消」与「保存更新」并排放置在同一行（Cancel / Save Changes），视觉简洁规整；
     - **保存更新**：点击「保存更新」，执行数值与防超卖校验后持久化至本地 SQLite 数据库，并即时联动重算当前持仓量、加权均价与盈亏总额；
     - **取消更新**：点击「取消」或左上角关闭按钮，直接收起编辑弹窗并放弃当前编辑态，不影响任何原始记录。
  4. **删除交易流水**：
     - 编辑弹窗右上角提供删除（红色垃圾桶图标）入口，避免在底部与保存混淆；
     - 点击后触发二次防误删确认对话框，警示用户「删除后无法恢复，将重新计算持仓与成本盈亏」；
     - 确认删除后从数据库中永久移除该笔流水，平滑关闭弹窗，并实时更新资产看板与详情页面。
